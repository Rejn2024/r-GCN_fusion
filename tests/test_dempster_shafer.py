...
def test_subset_masks_use_singletons_binaries_plus_uncertainty_for_more_than_ten_hypotheses():
    expected_binary_masks = [
        (1 << left_idx) | (1 << right_idx)
        for left_idx in range(11)
        for right_idx in range(left_idx + 1, 11)
    ]
    assert subset_masks(11) == [1 << idx for idx in range(11)] + expected_binary_masks + [(1 << 11) - 1]


def test_belief_plausibility_accepts_singleton_binary_uncertainty_masses_for_more_than_ten_hypotheses():
    hypotheses = [f"h{i}" for i in range(11)]
    intervals = belief_plausibility([0.75] + [0.0] * 10 + [0.0] * 55 + [0.25], hypotheses)
...
def test_combine_masses_accepts_singleton_binary_uncertainty_vectors():
    combined = combine_masses([0.7, 0.2] + [0.0] * 9 + [0.0] * 55 + [0.1], [0.6, 0.3] + [0.0] * 9 + [0.0] * 55 + [0.1])
    assert np.isclose(combined.sum(), 1.0)
    assert combined.shape == (67,)
